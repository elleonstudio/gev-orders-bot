...
В конце каждого расчета доступны кнопки Airtable, Excel и Notion."""
    ...
    if update.message:
        await update.message.reply_text(menu_text, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(kb))
    else:
        await update.callback_query.edit_message_text(menu_text, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(kb))
...
**1. Выкуп из текста (/paste или /calc)**
...
**2. Интерактивный выкуп (/zakaz)**
Напишите `/zakaz Zaven` и бот сам спросит название, количество, цены и размеры шаг за шагом.

**3. Фулфилмент (/ff)**
Запускается ТОЛЬКО после /paste или /zakaz. Позволяет собрать товары в наборы, выбрать платную упаковку из Notion и автоматически распределить всё по коробкам (максимум 30 кг в одной).

**4. Логистика Карго (/cargo)**
Независимый модуль. Напишите `/cargo`, создайте партию. 
Бот спросит упаковку (Мешок/Уголки/Обрешетка) и автоматически накинет вес тары (+1 кг за картон, +10 кг за дерево). Считает чистую прибыль между тарифом Карго и тарифом Клиента.

**5. Доставка по РФ (/dostavka_new)**
Напишите `/dostavka_new`, введите имя клиента, выберите склад (например, Коледино), укажите количество коробок. Бот сам прибавит 7000₽ за забор груза и выдаст готовый счет в рублях и драмах."""
    
    kb = [[InlineKeyboardButton("⬅️ Назад в меню", callback_data='menu_back')]]
    await query.edit_message_text(guide_text, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(kb))
...
def parse_paste_text(text):
    ...
    for kw in keywords: text = re.sub(f"(?i)({kw})", r"\n\1", text)
    ...
    for line in text.split('\n'):
        ...
        elif 'название:' in l and current_item is not None: current_item['name'] = line[line.lower().find('название:') + 9:].strip().title()
...
async def calc_fill_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for idx, item in enumerate(orders[uid]['items']):
        if item.get('purchase', 0.0) == 0.0 or item.get('dims', (0,0,0)) == (0,0,0):
            missing_idx = idx; break
    if missing_idx == -1:
        await query.message.reply_text("✅ Все данные уже заполнены! Вывожу расчет...")
        await finalize_order(uid, query.message); return ConversationHandler.END
...
async def c_get_purchase(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id)
    try: orders[uid]['items'][orders[uid]['calc_missing_idx']]['purchase'] = float(update.message.text.replace(',', '.'))
...
async def c_get_dims(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text != '-':
        ...
        except: await update.message.reply_text("❌ Введи 4 числа через пробел (или '-'):"); return C_DIMS
    missing_idx = next((i for i, item in enumerate(orders[uid]['items']) if item.get('purchase', 0.0) == 0.0 or item.get('dims', (0,0,0)) == (0,0,0)), -1)
    if missing_idx == -1: await update.message.reply_text("✅ Данные собраны!"); await finalize_order(uid, update.message); return ConversationHandler.END
    orders[uid]['calc_missing_idx'] = missing_idx
    await update.message.reply_text(f"Цена закупки (CNY) для <b>{orders[uid]['items'][missing_idx]['name']}</b>:", parse_mode='HTML'); return C_PURCHASE
...
async def cmd_zakaz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id)
    if not context.args: return await update.message.reply_text("❌ Напиши имя клиента после команды. Например: /zakaz Zaven8291")
...
async def z_get_dims(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Еще товар?", reply_markup=InlineKeyboardMarkup(kb)); return Z_MORE
...
async def z_more_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("Укажи курс клиенту (например 58):"); return Z_CLIENT_RATE

async def z_client_rate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id); orders[uid]['client_rate'] = float(re.findall(r'\d+\.?\d*', update.message.text.replace(',', '.'))[0]); await update.message.reply_text("Реальный курс закупа (например 55):"); return Z_REAL_RATE
...
async def cmd_ff(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id)
    if uid not in orders or not orders[uid].get('items'): return await update.message.reply_text("Сначала /paste или /zakaz"); ConversationHandler.END
...
async def show_ff_main_menu(update_or_query, uid):
    items = orders[uid]['items']; items_in_bundles = orders[uid]['ff_items_in_bundles']; bundles = orders[uid]['ff_bundles']
    msg = "📦 <b>FF Китай — Выбор режима</b>\n\n<b>Товары:</b>\n"
    for idx, item in enumerate(items): msg += f"{'☑️ <s>' if idx in items_in_bundles else '☐ '}{item['name']}{'</s>' if idx in items_in_bundles else ''}\n"
...
async def ff_main_menu_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'ff_back_menu': return await show_ff_main_menu(query, uid)
        ...
        orders[uid]['ff_bundle_selected'] = set(); orders[uid]['ff_bundle_available'] = available
        ...
        if missing:
            ...
            orders[uid]['ff_missing_idx'] = unpacked_indices[0]; return F_SINGLE_DIMS
        await query.edit_message_text("Напиши стоимость сборки/работы для ВСЕХ оставшихся одиночных товаров суммарно (¥) или 0:"); return F_SUMMARY

async def ff_single_dims(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id); nums = tuple(map(float, update.message.text.replace(',', '.').split())); idx = orders[uid]['ff_missing_idx']
    if len(nums) >= 3: orders[uid]['items'][idx]['dims'] = (nums[0], nums[1], nums[2])
...
async def show_bundle_item_selection(update_or_query, uid):
    available = orders[uid]['ff_bundle_available']; selected = orders[uid]['ff_bundle_selected']
    ...
    await update_or_query.edit_message_text("Выбери товары для набора:", reply_markup=InlineKeyboardMarkup(keyboard)); return F_BUNDLE_CREATE
...
async def ff_bundle_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith('ff_b_sel_'):
        ...
        return await show_bundle_item_selection(type('obj', (object,), {'edit_message_text': query.edit_message_text}), uid)
    elif query.data == 'ff_b_next': await query.edit_message_text("Введи имя набора:"); return F_BUNDLE_NAME

async def ff_bundle_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id); orders[uid]['ff_b_name'] = update.message.text.strip(); await update.message.reply_text("Размеры ОДНОГО готового набора (Д Ш В Вес):"); return F_BUNDLE_DIMS

async def ff_bundle_dims(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id); nums = tuple(map(float, update.message.text.replace(',', '.').split()))
    ...
    packages = await get_packages_from_notion(); orders[uid]['ff_available_packages'] = packages
    ...
    await update.message.reply_text("Выбери пакет:", reply_markup=InlineKeyboardMarkup(keyboard)); return F_BUNDLE_PACKAGE

async def ff_bundle_package_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer(); uid = str(update.effective_user.id)
    if query.data == 'ff_b_custom': await query.edit_message_text("Цена пакета (¥):"); return F_BUNDLE_PACKAGE
...
async def ff_bundle_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id); orders[uid]['ff_b_pkg'] = {'name': 'Ручной', 'price': float(update.message.text.replace(',', '.'))}; await update.message.reply_text("Цена сборки ЗА 1 НАБОР (¥):"); return F_BUNDLE_WORK

async def ff_bundle_work(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id); work = float(update.message.text.replace(',', '.'))
    orders[uid]['ff_bundles'].append({'name': orders[uid]['ff_b_name'], 'dims': orders[uid]['ff_b_dims'], 'weight': orders[uid].get('ff_b_weight', 0), 'qty': orders[uid]['ff_b_qty'], 'pkg': orders[uid]['ff_b_pkg'], 'work_price': work, 'item_indices': list(orders[uid]['ff_bundle_selected'])})
...
async def ff_summary_work(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id); single_work = float(update.message.text.replace(',', '.'))
    ...
    total_boxes = len(boxes); total_weight = sum(b['cur_weight'] for b in boxes); cost = total_boxes * BOX_PRICE_CNY
    ...
    await update.message.reply_text(res, parse_mode='HTML', reply_markup=InlineKeyboardMarkup(kb)); return ConversationHandler.END
...
async def cmd_dostavka(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id)
    if uid not in orders: return await update.message.reply_text("Сначала /zakaz или /paste")
    orders[uid]['warehouses'] = []
    keyboard = [[InlineKeyboardButton(c, callback_data=f'd_wh_{c}')] for c in TARIFFS.keys()]
    await update.message.reply_text("Выбери склад РФ:", reply_markup=InlineKeyboardMarkup(keyboard)); return D_WAREHOUSE

async def d_warehouse_cb(update, context):
    query = update.callback_query; await query.answer(); uid = str(update.effective_user.id)
    orders[uid]['current_wh'] = query.data.replace('d_wh_', ''); await query.edit_message_text("Количество коробок:"); return D_BOXES

async def d_boxes(update, context):
    uid = str(update.effective_user.id); boxes = int(update.message.text.strip()); city = orders[uid]['current_wh']
    ...
    await update.message.reply_text("Еще склад?", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Да", callback_data='d_more_yes'), InlineKeyboardButton("Нет", callback_data='d_more_no')]])); return D_MORE_WH

async def d_more_cb(update, context):
    query = update.callback_query; await query.answer()
    if query.data == 'd_more_yes':
        await query.edit_message_text("Склад РФ:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(c, callback_data=f'd_wh_{c}')] for c in TARIFFS.keys()])); return D_WAREHOUSE
    await query.edit_message_text("Курс ₽→драм:"); return D_RUB_RATE

async def d_rub_rate(update, context):
    uid = str(update.effective_user.id); orders[uid]['rub_rate'] = float(update.message.text.replace(',', '.'))
    ...
    kb = [[InlineKeyboardButton("📊 Excel Инвойс", callback_data='gen_excel')], [InlineKeyboardButton("📑 Export Airtable", callback_data='export_airtable')], [InlineKeyboardButton("💾 Обновить Notion", callback_data='paste_save_direct')]]
    await update.message.reply_text(msg, parse_mode='HTML', reply_markup=InlineKeyboardMarkup(kb)); return ConversationHandler.END
...
async def dn_get_boxes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    except: await update.message.reply_text("Введи число:"); return DN_BOXES
    
    city = orders[uid]['dn_current_wh']
    if city == 'Свой тариф': await update.message.reply_text("Отмена. Выбери другой тариф через команду заново."); return ConversationHandler.END
...
async def dn_get_rate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id)
    try: rate = float(update.message.text.replace(',', '.'))
    except: await update.message.reply_text("Введи число:"); return DN_RATE
...
async def cmd_cargo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text:
        ...
        if not active_parties: return await update.message.reply_text("📂 Активных партий Карго нет. Чтобы создать, напиши /cargo и список товаров.")
        msg = "📂 **Ваши активные партии в Китае:**\n\n"
        keyboard = []
        for cid, draft in active_parties.items():
            ready = sum(1 for i in draft['items'] if i['pieces'] > 0); total = len(draft['items'])
    ...
    cargo_id = generate_cargo_id(); data['cargo_id'] = cargo_id; cargo_drafts[uid][cargo_id] = data
...
async def cg_open_draft(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer(); uid = str(update.effective_user.id)
    cid = query.data.replace('cg_open_', ''); draft = cargo_drafts[uid].get(cid)
    ...
    if ready < total:
        ...
        t_pieces = sum(i['pieces'] for i in draft['items']); density = int(t_weight / t_vol) if t_vol > 0 else 0
...
async def cg_fill_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if missing_idx == -1: return await query.message.reply_text("✅ Все данные заполнены! Открой партию заново.")
    orders[uid]['cg_missing_idx'] = missing_idx; item_name = draft['items'][missing_idx]['name']
    ...
    await query.message.reply_text(f"Выбери тип упаковки для **{item_name}**:", parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(kb)); return CG_PACK
...
async def cg_pack_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(msg, parse_mode='Markdown'); return CG_DIMS
...
async def cg_dims_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for line in update.message.text.split('\n'):
        ...
        try:
            ...
            if len(nums) < 5: return await update.message.reply_text(f"❌ Ошибка в строке: `{line}`. Нужно 5 цифр: Кол-во Вес Д Ш В", parse_mode='Markdown')
        ...
        except: return await update.message.reply_text(f"❌ Ошибка чтения строки: `{line}`", parse_mode='Markdown')
        
    cargo_drafts[uid][cid]['items'][idx].update({'pieces': total_pieces, 'weight': total_weight, 'dims': (total_vol, 1, 1)}) # Сохраняем кубы в первом слоте dims для простоты
    
    missing_idx = next((i for i, item in enumerate(cargo_drafts[uid][cid]['items']) if item['pieces'] == 0), -1)
    if missing_idx == -1: await update.message.reply_text("✅ Все габариты заполнены! Открой /cargo для сводки."); return ConversationHandler.END
    orders[uid]['cg_missing_idx'] = missing_idx
    kb = [[InlineKeyboardButton("🟡 Мешок ($5)", callback_data='cg_pack_sack')], [InlineKeyboardButton("📦 Уголки ($6)", callback_data='cg_pack_corners')], [InlineKeyboardButton("🪵 Обрешетка ($8)", callback_data='cg_pack_wood')]]
    await update.message.reply_text(f"Выбери тип упаковки для товара **{cargo_drafts[uid][cid]['items'][missing_idx]['name']}**:", parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(kb)); return CG_PACK

async def cg_calc_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer(); await update.callback_query.message.reply_text("1️⃣ Введи Тариф Карго (себестоимость, $/кг):"); return CG_T_CARGO
async def cg_t_cargo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    str(update.effective_user.id); orders[str(update.effective_user.id)]['cg_tc'] = float(update.message.text.replace(',', '.')); await update.message.reply_text("2️⃣ Введи Тариф для Клиента ($/кг):"); return CG_T_CLIENT
async def cg_t_client(update: Update, context: ContextTypes.DEFAULT_TYPE):
    str(update.effective_user.id); orders[str(update.effective_user.id)]['cg_tcl'] = float(update.message.text.replace(',', '.')); await update.message.reply_text("3️⃣ Введи Курс USD → CNY:"); return CG_R_CNY
async def cg_r_cny(update: Update, context: ContextTypes.DEFAULT_TYPE):
    str(update.effective_user.id); orders[str(update.effective_user.id)]['cg_rcny'] = float(update.message.text.replace(',', '.')); await update.message.reply_text("4️⃣ Введи Курс CNY → AMD:"); return CG_R_AMD
...
async def cg_r_amd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    t_weight = sum(i['weight'] for i in draft['items'])
    t_vol = sum(i['dims'][0] for i in draft['items']) # Кубы мы сохранили в dims[0]
    ...
    client_weight_usd = t_weight * orders[uid]['cg_tcl']; client_total_usd = client_weight_usd + pack_cost + unload_cost
    ...
    await update.message.reply_text(msg_admin, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(kb)); return ConversationHandler.END
...
async def export_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'gen_excel':
        ...
        cid = orders[uid].get('active_cargo_id'); draft = cargo_drafts[uid][cid]
        ...
        cid = orders[uid].get('active_cargo_id'); draft = cargo_drafts[uid][cid]
        items_data = []
        for i, item in enumerate(draft['items'], 1): items_data.append({"№": i, "Название товара": item['name'], "Упаковка": item['pack_type'], "Места (шт)": item['pieces'], "Вес (кг)": item['weight'], "Объем (м³)": item['dims'][0]})
        ...
        items_data = []
        for w in orders[uid]['dn_wh_list']: items_data.append({"Описание услуги": f"Доставка на склад: {w['city']}", "Количество": f"{w['boxes']} коробок", "Тариф (RUB)": f"{TARIFFS[w['city']]} ₽", "Сумма (RUB)": f"{w['cost']} ₽"})
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            writer.sheets['Dostavka_Invoice'].set_column('A:A', 35); writer.sheets['Dostavka_Invoice'].set_column('B:D', 18)
        output.seek(0); await context.bot.send_document(chat_id=query.message.chat_id, document=InputFile(output, filename=f"Dostavka_{orders[uid]['dn_client']}.xlsx"))

    elif query.data == 'dn_delete':
        await query.edit_message_text(f"{query.message.text}\n\n✅ Расчет отменен и удален.")

def cancel(update, context): update.message.reply_text("Действие отменено."); return ConversationHandler.END
...
def main():
    ...
    app.add_handler(ConversationHandler(
        ...
        states={
            F_MAIN_MENU: [CallbackQueryHandler(ff_main_menu_cb)], F_SINGLE_DIMS: [MessageHandler(filters.TEXT & ~filters.COMMAND, ff_single_dims)],
            F_BUNDLE_CREATE: [CallbackQueryHandler(ff_bundle_cb)], F_BUNDLE_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, ff_bundle_name)],
            F_BUNDLE_DIMS: [MessageHandler(filters.TEXT & ~filters.COMMAND, ff_bundle_dims)], F_BUNDLE_PACKAGE: [CallbackQueryHandler(ff_bundle_package_cb), MessageHandler(filters.TEXT & ~filters.COMMAND, ff_bundle_price)],
            F_BUNDLE_WORK: [MessageHandler(filters.TEXT & ~filters.COMMAND, ff_bundle_work)], F_SUMMARY: [MessageHandler(filters.TEXT & ~filters.COMMAND, ff_summary_work)],
        }, fallbacks=[CommandHandler('cancel', cancel)]
    ))

    app.add_handler(ConversationHandler(
        entry_points=[CommandHandler('dostavka', cmd_dostavka)],
        states={D_WAREHOUSE: [CallbackQueryHandler(d_warehouse_cb)], D_BOXES: [MessageHandler(filters.TEXT & ~filters.COMMAND, d_boxes)], D_MORE_WH: [CallbackQueryHandler(d_more_cb)], D_RUB_RATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, d_rub_rate)]},
        fallbacks=[CommandHandler('cancel', cancel)]
    ))
    
    app.add_handler(ConversationHandler(
        entry_points=[CommandHandler('dostavka_new', cmd_dostavka_new)],
        states={DN_CLIENT: [MessageHandler(filters.TEXT & ~filters.COMMAND, dn_get_client)], DN_WH: [CallbackQueryHandler(dn_warehouse_cb)], DN_BOXES: [MessageHandler(filters.TEXT & ~filters.COMMAND, dn_get_boxes)], DN_MORE: [CallbackQueryHandler(dn_more_cb)], DN_RATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, dn_get_rate)]},
        fallbacks=[CommandHandler('cancel', cancel)]
    ))
    ...
    app.add_handler(ConversationHandler(
        ...
        states={CG_PACK: [CallbackQueryHandler(cg_pack_cb, pattern='^cg_pack_')], CG_DIMS: [MessageHandler(filters.TEXT & ~filters.COMMAND, cg_dims_input)]},
        fallbacks=[CommandHandler('cancel', cancel)]
    ))
    
    app.add_handler(ConversationHandler(
        entry_points=[CallbackQueryHandler(cg_calc_start, pattern='^cg_calc$')],
        states={CG_T_CARGO: [MessageHandler(filters.TEXT & ~filters.COMMAND, cg_t_cargo)], CG_T_CLIENT: [MessageHandler(filters.TEXT & ~filters.COMMAND, cg_t_client)], CG_R_CNY: [MessageHandler(filters.TEXT & ~filters.COMMAND, cg_r_cny)], CG_R_AMD: [MessageHandler(filters.TEXT & ~filters.COMMAND, cg_r_amd)]},
        fallbacks=[CommandHandler('cancel', cancel)]
    ))
    ...
    logger.info("Бот запущен. Версия v61 (Dostavka_New + Guide)")
    app.run_polling()

...
def parse_paste_text(text):
    ...
    for line in text.split('\n'):
        ...
        if 'клиент:' in l: 
            ...
            idx = line.lower().find('название:') + len('название:')
            current_item['name'] = line[idx:].strip().title()
...
# ======== ОБЩИЙ ОБРАБОТЧИК КНОПОК ТОВАРОВ ========
...
async def ff_bundle_work(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    query_mock = type('obj', (object,), {'edit_message_text': update.message.reply_text}); return await show_ff_main_menu(query_mock, uid)
...
async def cg_dims_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if pack_type == 'Уголки': weight_per_piece += 1.0 # +1 кг на картон
        elif pack_type == 'Обрешетка': weight_per_piece += 10.0; l += 5; w += 5; h += 5 # дерево
            
        cargo_drafts[uid][cid]['items'][idx].update({'pieces': pieces, 'weight': pieces * weight_per_piece, 'dims': (l, w, h)})
    except:
...
async def cg_calc_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer(); await query.message.reply_text("1. Введи тариф Карго (Себестоимость $/кг):"); return CG_T_CARGO
async def cg_t_cargo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id); orders[uid]['cg_tc'] = float(update.message.text.replace(',', '.')); await update.message.reply_text("2. Введи тариф для Клиента ($/кг):"); return CG_T_CLIENT
async def cg_t_client(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id); orders[uid]['cg_tcl'] = float(update.message.text.replace(',', '.')); await update.message.reply_text("3. Введи курс Карго (USD → CNY):"); return CG_R_CNY
async def cg_r_cny(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id); orders[uid]['cg_rcny'] = float(update.message.text.replace(',', '.')); await update.message.reply_text("4. Введи курс Драма (CNY → AMD):"); return CG_R_AMD
...
**1. ОТДАЕМ В КАРГО:**
• Себестоимость Карго (${orders[uid]['cg_tc']}/кг + Услуги): **${cargo_total_usd:.1f}**
...
async def cg_export_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'cg_export_air':
        ...
        for i, item in enumerate(draft['items'], 1):
            items_data.append({"№": i, "Название товара": item['name'], "Упаковка": item['pack_type'], "Места (шт)": item['pieces'], "Вес (кг)": item['weight'], "Объем (м³)": (item['pieces'] * (item['dims'][0]*item['dims'][1]*item['dims'][2])/1000000)})
